// The native client task 3 — AoidNativeFlow, the ceremony state machine.
//
...
//      submitted credential (D3's precondition for the widgets layer's sealed widgets),
...
void main() {
  ...
  test(
    ...
    () async {
      ...
      // The issuer folds unknown-email / wrong-password / no-credential / locked into
  ...
  test(
    ...
    () async {
      ...
      // The issuer's write gate fires BEFORE the service is called, so the handle was
  ...
  test(
    ...
    () async {
      // The issuer's MaxAttempts is DURABLE and per-handle. A client retry loop burns
  ...
  group('9 the handle LIFECYCLE on failure paths', () {
    ...
    test('a 503 write-gate rejection does NOT consume the handle', () async {
      // The issuer's nativeWriteAllowed runs BEFORE r.nativeLogin.Verify, so the
    ...
    test(
      ...
      () async {
        // The issuer: Consume burns the presented handle and increments the DURABLE
    ...
    test(
      ...
      () async {
        // The client-side counterpart of mutation 11 (reusing the
    ...
    test(
      ...
      () async {
        ...
        // handle is gone, and the issuer's attempt cap is durable.
  ...
  group('8 D3 source gate — no credential read-back', () {
    ...
    // grep cannot tell a doctrine comment from a declaration (the lesson the issuer
    // and the issuer both recorded).
    ...
    test('the flow declares no credential getter', () {
      ...
      expect(
        ...
        reason:
            ...
            'plaintext back. The widgets layer seals AoidLoginForm on this guarantee.',
      );
    });
